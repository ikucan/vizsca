...
object plot_spreads_by_deaaler {
  ...
  def main(argv: Array[String]): Unit = {
    ...
    val dates = (1 to 5) map (i => f"201907${i}%02d")
    ...
    if (1 > 0) {
      ...
      pp + ttl(raw"spreads")
      pp + hln(Array(0.0),clr(0xff000000))

      val dlrs = df.dlr[String]
      val clrs = df.ttp[String]
      val hrs = df.hrs_mask[Boolean]
      val cme_idx = clrs((x: String) => x == "CME")
      val lch_idx = clrs((x: String) => x == "LCH")

      val ddlrs = dlrs.dstnct

      (0 until 1) foreach { i =>
        //(0 until ddlrs.size) foreach { i =>
        val dlr = ddlrs(i)
        println(dlr)
        val dlr_idx = dlrs((x: String) => x == dlr)
        val dlr_cme = df.slct(hrs && dlr_idx && cme_idx)
        val dlr_lch = df.slct(hrs && dlr_idx && lch_idx)

        pp + sctr2(dlr_cme.st[Long], dlr_cme.sprd[Double], strk = clr(0xff3333cc), fll = clr(0x662222aa), shp = pls, sz = 8f, ln_sz = 1f)
        pp + sctr2(dlr_cme.st[Long], dlr_cme.twa_sprd[Double], strk = clr(0xffcc3333), fll = clr(0x66aa1111), shp = dot, sz = 8f, ln_sz = 1f)
